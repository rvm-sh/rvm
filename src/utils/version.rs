...
pub fn is_version_installed(runtime_name: &str, version: &str) -> Result<bool> {
    display_step(&format!("Checking if {} {} is already installed", runtime_name, version));
    ...
    if is_installed {
        display_success(&format!("{} {} is already installed", runtime_name, version));
    } else {
        display_success(&format!("{} {} is not installed, proceeding", runtime_name, version));
    }
...
pub fn remove_version(runtime_name: &str, version: &str) -> Result<()> {
    display_step(&format!("Removing {} {} from filesystem", runtime_name, version));
    ...
    display_step(&format!("Removing {:.1}MB of files", dir_size as f64 / 1024.0 / 1024.0));
    
    // Remove the version directory
    std::fs::remove_dir_all(&version_dir)?;
    
    display_success(&format!("Removed {} {} ({:.1}MB freed)", runtime_name, version, dir_size as f64 / 1024.0 / 1024.0));
    Ok(())
}
...
pub fn resolve_installed_version(runtime_name: &str, version_input: &str) -> Result<String> {
    ...
    if installed_versions.is_empty() {
        return Err(RvmError::VersionNotFound(format!("No {} versions installed", runtime_name)));
    }
    ...
    if version_input.chars().all(|c| c.is_ascii_digit()) {
        ...
        for installed in &installed_versions {
            if installed.starts_with(&target_major) && installed.chars().nth(target_major.len()) == Some('.') {
    ...
    display_error(&format!("{} {} is not installed", runtime_name, version_input));
...
pub fn apply_version_to_current_session(runtime_name: &str, version: &str, binary_name: &str) -> Result<()> {
