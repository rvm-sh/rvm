...
# Define the default and latest versions of yarn
DEFAULT_YARN_VERSION=""
ENABLE_AUTOCHECK="false"  # Control auto version check

INSTALLED_VERSIONS=""

# Function to handle yarn commands
yarn() {
    ...
    if [ "$ENABLE_AUTOCHECK" = "true" ]; then
        ...
        if [ -f "package.json" ]; then
            ...
            if ! command -v jq > /dev/null 2>&1; then
                ...
                use_yarn_version "$DEFAULT_YARN_VERSION" "$@"
                return
            fi
            
            # Extract the yarn version from package.json
            local yarn_version_specified
            yarn_version_specified=$(jq -r '.engines.yarn // empty' package.json)

            if [ -z "$yarn_version_specified" ]; then
                echo "No yarn version specified in package.json. Using the default version."
                use_yarn_version "$DEFAULT_YARN_VERSION" "$@"
                return
            fi

            # Determine the yarn version to use
            local version_to_use
            version_to_use=$(determine_yarn_version "$yarn_version_specified")
            use_yarn_version "$version_to_use" "$@"
            return
        else
            echo "package.json not found. Using the default yarn version."
        fi
    fi
...
# Function to use a specific yarn version
use_yarn_version() {
    local version="$1"
    ...
    local yarn_executable="${HOME}/.yarn/v${version}/yarn"

    if [ -f "$yarn_executable" ]; then
        echo "Using yarn version: $version"
        $yarn_executable "$@"
    else
        echo "yarn version $version not found. Please install this version before running the command."
        return 1
    fi
}

# Function to determine the yarn version to use based on the specification
determine_yarn_version() {
    ...
    # Specified greater than major version (e.g., ">=8")
        ...
        echo determined_version="$DEFAULT_YARN_VERSION"
        return
    fi
